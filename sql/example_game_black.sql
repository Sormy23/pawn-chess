...
exec playTurn(from_square => 'f2', to_square => 'f4');

SELECT * FROM board;

-- Player 2 (Black): Move pawn from e7 to e5
exec playTurn(from_square => 'd5', to_square => 'e4');

SELECT * FROM board;

-- Player 1 (White): Move pawn from e4 to d5 (en passant)
exec playTurn(from_square => 'f4', to_square => 'f5');

SELECT * FROM board;

-- Player 2 (Black): Move pawn from f7 to f6
exec playTurn(from_square => 'e4', to_square => 'e3');

SELECT * FROM board;

-- Player 1 (White): Move pawn from e6 to e7
exec playTurn(from_square => 'f5', to_square => 'f6');

SELECT * FROM board;

-- Player 2 (Black): Move pawn from f6 to f5
exec playTurn(from_square => 'e3', to_square => 'e2');

SELECT * FROM board;

-- Player 1 (White): Move pawn from c6 to d7 (capture)
exec playTurn(from_square => 'f6', to_square => 'e7');

SELECT * FROM board;

exec playTurn(from_square => 'e2', to_square => 'e1');